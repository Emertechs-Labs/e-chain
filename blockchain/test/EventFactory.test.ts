...
describe("EventFactory", function () {
  ...
  describe("Organizer Management", function () {
    ...
    it("Should reject non-owner attempts to verify organizers", async function () {
      await expect(
        eventFactory.connect(user).verifyOrganizer(organizer.address)
      ).to.be.revertedWithCustomError(eventFactory, "OwnableUnauthorizedAccount");
    });
  });
  ...
  describe("Event Creation", function () {
    ...
    it("Should reject event creation from unverified organizers", async function () {
      await expect(
        eventFactory.connect(user).createEvent(
          "Test Event",
          "ipfs://QmTest123",
          ethers.parseEther("0.1"),
          100,
  ...
  describe("Event Management", function () {
    ...
    it("Should allow organizer to update event", async function () {
      ...
      await expect(
        eventFactory.connect(organizer).updateEvent(1, newName, newMetadataURI)
    ...
    it("Should allow organizer to change event status", async function () {
      await expect(
        eventFactory.connect(organizer).setEventStatus(1, false)
    ...
    it("Should reject updates from non-organizers", async function () {
      await expect(
        eventFactory.connect(user).updateEvent(1, "Hacked Event", "ipfs://QmHacked")
